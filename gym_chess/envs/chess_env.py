...
class ChessEnv(gym.Env):
    ...
    def _get_array_state(self):
        ...
        moves_array = np.zeros(len(self.all_possible_actions))
        legal_moves = self._get_legal_move_list()
        for move in legal_moves:
            if move in self.all_possible_actions:
                moves_array[self.all_possible_actions.index(move)] = 1
        return [state, moves_array]
